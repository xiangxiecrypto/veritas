 ...
 * @notice Accepts callbacks from Primus network
 * @dev PrimusTask contract calls the callback, not the attestor directly
 */
contract PrimusVeritasAppV4Callback {
    address public owner;
    VeritasValidationRegistryV4 public immutable registry;
    
    // PrimusTask contract on Base Sepolia
    address public constant PRIMUS_TASK = 0xC02234058caEaA9416506eABf6Ef3122fCA939E8;
    
    // Attestor address for verification
    address public constant PRIMUS_ATTESTOR = 0x0DE886e31723e64Aa72e51977B14475fB66a9f72;
    
    // Authorized callers (can be updated by owner)
    ...
    event AttestationProcessed(bytes32 indexed hash, address caller, string data, uint8 score);
    ...
    constructor(address _registry) {
        ...
        // Authorize known Primus addresses
        authorizedCallers[PRIMUS_TASK] = true;
        authorizedCallers[PRIMUS_ATTESTOR] = true;
    }
    ...
    /**
     * @notice Process attestation from Primus network
     * @dev Can be called by PrimusTask, authorized callers, or owner
     */
    function processAttestation(
        ...
        emit AttestationProcessed(attestationHash, msg.sender, attestationData, response);
    }
