...
// TaskResult structure for Primus callback
struct TaskResult {
    address recipient;
    bytes request;
...
/**
 * @title PrimusVeritasAppV4Option2
 * @notice V4 App with PrimusTask callback integration
 * @dev Works with PrimusTaskMock for automated callbacks
 */
contract PrimusVeritasAppV4Option2 {
    address public owner;
    VeritasValidationRegistryV4 public immutable registry;
    
    // PrimusTask contract (can be updated by owner)
    address public primusTask;
    
    // Primus attestor (hardcoded for security)
    ...
    event PrimusTaskUpdated(address indexed newTask);
    ...
    modifier onlyPrimusTask() {
        require(msg.sender == primusTask, "Only PrimusTask");
        _;
    }
    ...
    constructor(address _registry, address _primusTask) {
        ...
        primusTask = _primusTask;
    }
    
    function setPrimusTask(address _primusTask) external onlyOwner {
        primusTask = _primusTask;
        emit PrimusTaskUpdated(_primusTask);
    }
    ...
    /**
     * @notice Primus callback - called by PrimusTask after attestation
     */
    function reportTaskResultCallback(
        bytes32 taskId,
        TaskResult calldata taskResult,
        bool success
    ) external onlyPrimusTask {
        require(success, "Attestation failed");
        require(!processedTasks[taskId], "Already processed");
        
        // Verify recipient is this contract
        require(taskResult.recipient == address(this), "Invalid recipient");
        
        _processAttestation(taskId, taskResult.data, taskResult.timestamp, 0);
    }
