 ...
 */
async function getPrimusAttestors(provider: ethers.providers.Provider): Promise<string[]> {
  ...
  const nodeRegistry = new ethers.Contract(
    PRIMUS_NETWORK_CONTRACTS.nodeRegistry,
    PRIMUS_NODE_ABI,
    provider
  );
 ...
 */
async function getAttestorsFromTaskContract(
  ...
  const eventSignature = ethers.utils.id('AttestationSubmitted(bytes32,address,bytes)');
  ...
  for (const log of logs) {
    ...
    if (log.topics[2]) {
      const attestor = ethers.utils.getAddress('0x' + log.topics[2].slice(26));
 ...
 */
async function verifyAttestationWithDynamicAttestorCheck(
...
async function giveFeedbackWithVerificationExample() {
  ...
  const verification = await verifyAttestationWithDynamicAttestorCheck(
    primusNetwork,
    attestationResult as any,
    provider
  );
...
export {
  getPrimusAttestors,
  getAttestorsFromTaskContract,
  verifyAttestationWithDynamicAttestorCheck
};
